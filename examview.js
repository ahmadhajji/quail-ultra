...
function applyAssetPaths(container, maxHeight) {
  const apppath = window.document.URL.match(/(.+)\/examview.html/)[1]
  const qbpath = url.pathToFileURL(localinfo.path).toString()

  container.find('img').each(function() {
    this.src = this.src.replace(apppath, qbpath)
  ...
  container.find('audio').each(function() {
    if (this.src === '') {
      ...
      if (source && source.src) {
        this.src = source.src.replace(apppath, qbpath)
      }
    } else {
      this.src = this.src.replace(apppath, qbpath)
    }
  ...
  container.find('a').each(function() {
    if (this.href !== '') {
      this.href = this.href.replace(apppath, qbpath)
    }
  })
}
...
function extractChoiceLabels() {
  ...
  const rawText = cloned.text().replace(/\r/g, '')
  const choiceRegex = /(?:^|\n)\s*([A-Z])[\)\.]\s*(.+?)(?=(?:\n\s*[A-Z][\)\.]\s)|$)/gs
  let match
